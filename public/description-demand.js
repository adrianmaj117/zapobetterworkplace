...
(() => {
  ...
  const excludedFromShopping = (category, text = '') => {
    ...
    return group === 'inne' || group.includes('owoce') || group.includes('warzywa') || (group.includes('bulki') && group.includes('katowic')) || ignoredDemandLine(`${category} ${text}`);
  };
  ...
  function printShoppingList(list) {
    ...
    let defaultBarcode = document.querySelector('#selgrosCardDisplay')?.innerHTML || '';
    if (!/<(?:rect|path|line|g)\b/i.test(defaultBarcode) && window.JsBarcode) {
      ...
      defaultBarcode = svg.outerHTML;
    }
    // Nagłówek SELGROS jest już częścią karty w szablonie wydruku.
    // Wstawiamy tu wyłącznie kod, aby karta zachowała jeden czytelny nagłówek.
    const cardContent = defaultBarcode;
    win.document.write(`<!doctype html><html lang="pl"><head><meta charset="utf-8"><title>Lista zakupów — ZapoBetterWorkPlace</title><style>@page{size:A4;margin:15mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#173b2e;margin:0;background:#fff}.print-sheet{max-width:900px;margin:0 auto}.print-brand{text-align:center;border-bottom:2px solid #d9e8dd;padding:4px 0 19px;margin-bottom:20px}.print-brand img{width:158px;height:158px;object-fit:contain;border-radius:18px;display:block;margin:0 auto 10px}.print-brand strong{font-size:20px;letter-spacing:.1px}.print-brand p{margin:6px 0 0;color:#61766b;font-size:13px}.print-header{display:flex;align-items:flex-start;justify-content:space-between;gap:22px;margin-bottom:14px}.print-title h1{font-family:Georgia,serif;font-size:31px;margin:0}.print-date{margin:8px 0 0;color:#536b60;font-size:14px}.print-selgros{width:235px;flex:none;border:1px solid #d9e2dc;border-radius:11px;overflow:hidden;background:#fff;text-align:center;box-shadow:0 2px 8px #183b2e12}.print-selgros strong{display:block;background:#e30613;color:#fff;padding:9px;font-size:15px;letter-spacing:.08em}.print-selgros svg{display:block;width:100%;height:auto;margin:7px auto 9px}.print-selgros img{display:block;width:100%;height:150px;object-fit:contain;background:#fff}.print-selgros .nip{display:block;border-top:1px solid #e3e9e5;padding:7px 9px;color:#173b2e;font-size:12px;font-weight:700;letter-spacing:.03em}table{width:100%;border-collapse:separate;border-spacing:0;border:1px solid #ccdcd2;border-radius:12px;overflow:hidden;font-size:14px}th,td{border-bottom:1px solid #dfe8e2;text-align:left;padding:11px;vertical-align:top}tr:last-child td{border-bottom:0}th{background:#eaf3ed;color:#244637;font-size:12px;text-transform:uppercase;letter-spacing:.35px}td:nth-child(3),td:nth-child(4){white-space:nowrap}td:last-child b{color:#a3322d}small{color:#5d7168;line-height:1.5}.print-footer{text-align:center;color:#718178;font-size:11px;margin-top:22px}@media print{.print-sheet{max-width:none}.print-brand img{width:145px;height:145px}tr{break-inside:avoid}}</style></head><body><main class="print-sheet"><header class="print-brand"><img src="${logo}" alt="Daily Fruits by BetterWorkPlace"><strong>ZapoBetterWorkPlace</strong><p>Lista zakupów · magazyn spożywczy</p></header><section class="print-header"><div class="print-title"><h1>Lista zakupów</h1><p class="print-date">Na dzień: ${escapeHtml(list.list_date || list.created_at || '')}</p></div><aside class="print-selgros" aria-label="Karta SELGROS"><strong>SELGROS</strong>${cardContent}<span class="nip">NIP: 6793077034</span></aside></section><table><thead><tr><th>Kategoria</th><th>Produkt</th><th>Stan</th><th>Do kupienia</th></tr></thead><tbody>${rowsForPrint}</tbody></table><p class="print-footer">Wygenerowano z systemu Magazyn BetterWorkPlace</p></main></body></html>`);
