...
(() => {
  ...
  async function printShoppingList(list) {
    ...
    const savedCard = await api('/api/settings/selgros-card').catch(() => ({}));
    const defaultBarcode = document.querySelector('#selgrosCardDisplay')?.innerHTML || '';
    const cardContent = savedCard?.image_data ? `<img src="${savedCard.image_data}" alt="Karta SELGROS">` : defaultBarcode;
